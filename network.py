...
class ConvNeXtV2_Block(nn.Module):
    ...
    #dim int is the number of input channels
    #drop_path is thestochastic depth rate
...
class ResNet_Block(nn.Module):
    def __init__(self,width,fsize,dilv,nblock,use_convnext_v2_block=False, convnext_drop_path_ratio=0, use_self_attention=False):
        ...
        self.layer1 = Maxout2d(in_channels=width, out_channels=width, pool_size=4,
                               kernel_size=fsize, dilation=dilv, block=nblock, use_convnext_v2_block=use_convnext_v2_block,
                                 convnext_drop_path_ratio= convnext_drop_path_ratio)
        if not use_self_attention:
            self.attention = SCSE(width, 16)
        else:
            #  Self_Attention can change heads  dim_head and dropout # 
            # self.attention = Self_Attention(width, heads = 4, dim_head = 32, dropout = 0.)
            self.attention = CoordAtt(width, reduction=32)
...
class GRUResNet(nn.Module):
    def __init__(self,width,cwidth):
        ...
        layers = []
                    #check whether using covnext_v2_block
                    
        layer = Maxout2d(in_channels=NUM_CHANNELS+width+1, out_channels=cwidth, pool_size=3, 
                         use_convnext_v2_block=True,convnext_drop_path_ratio=0)
        ...
        for rep in range(16):
            for fsize,dilv in [(5,1)]:
                if fsize > 0:
                    #check whether using covnext_v2_block
                    #check whether using self-attention layer
                    layer = ResNet_Block(cwidth, fsize, dilv, nblock, 
                                         use_convnext_v2_block=True, 
                                         convnext_drop_path_ratio=0, use_self_attention =True)
                    layers.append(layer)
                    nblock += 1
